...
if ($_GET['api'])
{
	...
	{$s = shell_exec('curl --referer "https://google.com/" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0" --silent '.escapeshellarg($_POST['url']));}
		else
...
$s = file_get_contents('../dom/dom.php');
preg_match('#public function css\(#si', $s, $m, PREG_OFFSET_CAPTURE);
$s = substr($s, 0, $m[0][1]);
preg_match('#^.*/\*(.*?)\*/#si', $s, $m);
$help = trim($m[1]);
...
<body>
	<table width="100%">
		...
		<tr>
			<td>CSS селектор (или пусто) <a href="#" id=help title="">[ ? ]</a>: </td>
			<td>
